...
export function renderWeaponModifiers(item, weaponKeyMap = null) {
  ...
  const hasSkills =
    mods.axSkill || mods.befoulment || mods.weaponKeys.length > 0;
  if (hasSkills) {
...
export function renderStars(maxLevel, type) {
  ...
  for (const tier of config.tiers) {
    if (maxLevel >= tier.level)
      html += `<span class="star ${tier.cls}"></span>`;
  }
  html += "</span>";
  return html;
}
...
function renderBaseStats({
  ...
  if (element && GAME_ELEMENT_NAMES[element]) {
    html += statRow(
      t("stat_element"),
      `<img class="stat-icon" src="${getImageUrl(`labels/element/Label_Element_${GAME_ELEMENT_NAMES[element]}.png`)}" alt="${GAME_ELEMENT_NAMES[element]}">`,
    );
  }
  ...
  if (proficiencies?.length > 0) {
    const profIcons = proficiencies
      ...
      .map(
        (p) =>
          `<img class="stat-icon" src="${getImageUrl(`labels/proficiency/Label_Weapon_${GAME_PROFICIENCY_NAMES[p]}.png`)}" alt="${GAME_PROFICIENCY_NAMES[p]}">`,
      )
      .join("");
...
export function renderPartyDetail(container, data, options = {}) {
          ...
          <div class="job-skills-list">
            ${setAction
              .map((skill) => {
                ...
                const iconUrl = slug
                  ? getImageUrl(`job-skills/${slug}.png`)
                  : null;
                return `
          ...
          ${characters
            .map((item) => {
              ...
              const awakeningSlug = arousalForm
                ? CHARACTER_AWAKENING_MAPPING[arousalForm]
                : null;
              const hasPerpetuit = !!item.param?.has_npcaugment_constant;
              const hasModifiers =
                (awakeningSlug && awakeningSlug !== "character-balanced") ||
                hasPerpetuit;
  ...
  container.innerHTML =
    html || `<p class="cache-empty">${t("party_no_data")}</p>`;
}
